...
import { getEduTeachAchiOutcome } from '@/api/getAuditingData'
export default {
  name: 'EducationOutcomeTable',
  data() {
    return {
      token: getToken(),
      tableData: []
  ...
  methods: {
    getAuditingData: function() {
      getEduTeachAchiOutcome(this.token).then(response => {
        // // this.tableData = response.data
        console.log(response)
        console.log(response.data.eduTeaachiAuditResult)
        this.tableData = response.data.eduTeaachiAuditResult
      })
    }
  }
}
