...
import { getResearchCourseOutcome } from '@/api/getAuditingData'
export default {
  name: 'ResearchOutcomeTable',
  data() {
    return {
      token: getToken(),
      tableData: []
  ...
  methods: {
    getAuditingData: function() {
      getResearchCourseOutcome(this.token).then(response => {
        // // this.tableData = response.data
        console.log(response)
        console.log(response.data.resCourseAuditResult)
        this.tableData = response.data.resCourseAuditResult
      })
    }
  }
}
