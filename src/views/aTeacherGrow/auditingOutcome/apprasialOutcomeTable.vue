...
import { getEduTeachCompetOutcome } from '@/api/getAuditingData'
export default {
  name: 'ApprasialOutcomeTable',
  data() {
    return {
      token: getToken(),
      tableData: []
  ...
  methods: {
    getAuditingData: function() {
      getEduTeachCompetOutcome(this.token).then(response => {
        // // this.tableData = response.data
        console.log(response)
        console.log(response.data.eduTeachCmpAuditResult)
        this.tableData = response.data.eduTeachCmpAuditResult
      })
    }
  }
}
