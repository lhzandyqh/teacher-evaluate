...
import { getStuSubCompetOutcome } from '@/api/getAuditingData'
export default {
  name: 'StudentOutcomeTable',
  data() {
    return {
      token: getToken(),
      tableData: []
  ...
  methods: {
    getAuditingData: function() {
      getStuSubCompetOutcome(this.token).then(response => {
        // // this.tableData = response.data
        console.log(response)
        console.log(response.data.stuSubCmpAuditResult)
        this.tableData = response.data.stuSubCmpAuditResult
      })
    }
  }
}
