...
import { getTopicsOutcome } from '@/api/getAuditingData'
export default {
  name: 'ProjectOutcomeTable',
  data() {
    return {
      token: getToken(),
      tableData: []
  ...
  methods: {
    getAuditingData: function() {
      getTopicsOutcome(this.token).then(response => {
        // // this.tableData = response.data
        console.log(response)
        console.log(response.data.proTopicsAuditResults)
        this.tableData = response.data.proTopicsAuditResults
      })
    }
  }
}
