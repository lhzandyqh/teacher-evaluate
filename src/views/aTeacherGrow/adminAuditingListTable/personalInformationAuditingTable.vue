...
import { getPersonalInformationAuditingListData, adminAuditingPersonalInformation } from '@/api/adminAuditingInformation'
export default {
  name: 'PersonalInformationAuditingTable',
  data() {
    return {
      AuditingReason: '',
      auditingTeacherId: '',
  ...
  methods: {
    ...
    getAuditingList: function() {
      getPersonalInformationAuditingListData(this.token).then(response => {
        console.log('管理员测试教师个人信息审核列表')
        console.log(response.data)
        this.tableData = response.data.teacherInfoAuditResult
      })
    },
    beginAuditing: function(data) {
      console.log('检查每一条取得的data')
      console.log(data)
    ...
    auditingPass: function() {
      console.log('开始审核通过')
      console.log('检查id是否正确')
      console.log(this.auditingTeacherId)
      const prams = {
        verifyId: this.auditingTeacherId,
        status: '审核通过',
        description: this.AuditingReason
      }
      adminAuditingPersonalInformation({ ...prams, token: this.token }).then(response => {
        console.log('审核通过')
        this.$message({
          message: '审核成功',
          type: 'success'
        ...
        getPersonalInformationAuditingListData(this.token).then(response => {
          console.log('重新获取管理员测试教师个人信息审核列表')
          console.log(response.data)
          this.tableData = response.data.teacherInfoAuditResult
        })
      })
      this.jibenVisible = false
      this.xuexiVisible = false
    ...
    auditingNoPass: function() {
      ...
      console.log('测试审核原因')
      console.log(this.AuditingReason)
      this.AuditingReason = ''
      adminAuditingPersonalInformation({ ...prams, token: this.token }).then(response => {
        console.log('审核不通过')
        this.$message({
