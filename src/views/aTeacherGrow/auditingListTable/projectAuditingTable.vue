  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogPvVisible" title="审核详情">
      ...
      <el-table :data="pvData" fit highlight-current-row style="width: 100%">
        ...
        <el-table-column prop="item_setting_unit" label="立项单位"/>
        <el-table-column prop="topic_start_time" label="起始时间"/>
  ...
  methods: {
    getListData: function() {
      ...
      getAuditingListData({ ...prams, token: this.token }).then(response => {
        this.tableData = response.data.proTopicsAuditList
    ...
    getAuditing(id) {
      ...
      getProjectTopicsDetail({ ...prams, token: this.token }).then(response => {
        this.pvData = response.data.projectTopics
        console.log('输出PV数据')
    ...
    editAuditing: function(num) {
      ...
      auditingProjectTopics({ ...prams, token: this.token }).then(response => {
        this.$message({
          message: '审核完成',
          type: 'success'
        })
