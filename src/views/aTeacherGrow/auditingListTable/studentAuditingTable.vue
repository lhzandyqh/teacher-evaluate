  ...
  methods: {
    getListData: function() {
      ...
      getAuditingListData({ ...prams, token: this.token }).then(response => {
        this.tableData = response.data.guideStuSubCmpAchiCmpAuditList
    ...
    getAuditing(id) {
      ...
      getGuideStuSubAwardDetail({ ...prams, token: this.token }).then(response => {
        this.pvData = response.data.guideStuSubAward
        console.log('输出PV数据')
    ...
    editAuditing: function(num) {
      ...
      auditingGuideStuSubAward({ ...prams, token: this.token }).then(response => {
        this.$message({
          message: '审核完成',
          type: 'success'
        })
