  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogPvVisible" title="审核详情">
      ...
      <el-table :data="pvData" fit highlight-current-row style="width: 100%">
        <el-table-column prop="school_sponsors" label="校级发起人"/>
        <el-table-column prop="organizer_of_training" label="组织者"/>
...
import { getAuditingListData, getSchoolTrainDetail, auditingSchoolTrain } from '@/api/getAuditingData'
import imgPreview from '@/views/aTeacherGrow/auditingListTable/imgPreview'
export default {
  name: 'SchoolAuditingTable',
  ...
  methods: {
    ...
    getAuditing(id) {
      ...
      getSchoolTrainDetail({ ...prams, token: this.token }).then(response => {
        this.pvData = response.data.schoolTrain
        console.log('输出PV数据')
    ...
    editAuditing: function(num) {
      ...
      auditingSchoolTrain({ ...prams, token: this.token }).then(response => {
        this.$message({
          message: '审核完成',
          type: 'success'
        })
