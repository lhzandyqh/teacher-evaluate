<template>
  <div class="app-container">
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="300" align="center" label="课题名称">
        <template slot-scope="scope">
    ...
    <el-dialog :visible.sync="dialogPvVisible" title="审核详情">
      ...
      <el-table :data="pvData" fit highlight-current-row style="width: 100%">
        <el-table-column prop="topic_name" label="课题名称"/>
        <el-table-column prop="research_course_type" label="研究课类型"/>
        <el-table-column prop="organizers_of_research_course" label="主办单位"/>
        <!--<el-table-column prop="certType" label="证书颁发日期"/>-->
        <el-table-column prop="course_display_level" label="展示级别"/>
        <el-table-column prop="teaching_time" label="授课时间"/>
        <el-table-column prop="course_length" label="授课时长"/>
...
import { getAuditingListData, getResearchCourseDetail, auditingResearchCourse } from '@/api/getAuditingData'
import imgPreview from '@/views/aTeacherGrow/auditingListTable/imgPreview'
export default {
  name: 'ResearchAuditingTable',
  ...
  methods: {
    ...
    getListData: function() {
      ...
      getAuditingListData({ ...prams, token: this.token }).then(response => {
        this.tableData = response.data.resCourseCmpAuditList
    ...
    getAuditing(id) {
      ...
      getResearchCourseDetail({ ...prams, token: this.token }).then(response => {
        this.pvData = response.data.researchCourse
        console.log('输出PV数据')
    ...
    editAuditing: function(num) {
      ...
      auditingResearchCourse({ ...prams, token: this.token }).then(response => {
        this.$message({
          message: '审核完成',
          type: 'success'
        })
