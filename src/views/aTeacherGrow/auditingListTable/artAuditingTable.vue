  ...
  <div class="app-container">
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="300" align="center" label="比赛名称">
        <template slot-scope="scope">
          <span>{{ scope.row.competition_name }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogPvVisible" title="审核详情">
      ...
      <el-table :data="pvData" fit highlight-current-row style="width: 100%">
        <el-table-column prop="specific_name" label="比赛类别"/>
...
import { getAuditingListData, getArtSportsAssociationDetail, auditingArtSportsAssociation } from '@/api/getAuditingData'
...
export default {
  ...
  methods: {
    ...
    getAuditing(id) {
      ...
      getArtSportsAssociationDetail({ ...prams, token: this.token }).then(response => {
        this.pvData = response.data.artSportsAssociation
        console.log('输出PV数据')
    ...
    editAuditing: function(num) {
      ...
      auditingArtSportsAssociation({ ...prams, token: this.token }).then(response => {
        this.$message({
          message: '审核完成',
          type: 'success'
        })
