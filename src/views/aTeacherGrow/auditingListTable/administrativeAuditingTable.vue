<template>
  <div class="app-container">
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="300" align="center" label="获奖名称">
        <template slot-scope="scope">
          <span>{{ scope.row.award_name }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogPvVisible" title="审核详情">
      ...
      <el-table :data="pvData" fit highlight-current-row style="width: 100%">
        <el-table-column prop="award_name" label="获奖名称"/>
        ...
        <el-table-column prop="organizers_of_administrative" label="主办单位"/>
        <el-table-column prop="award_from" label="获奖形式"/>
        <el-table-column prop="award_types" label="获奖类型"/>
...
import { getAuditingListData, getAdministrativeDetail, auditingAdministrative } from '@/api/getAuditingData'
import imgPreview from '@/views/aTeacherGrow/auditingListTable/imgPreview'
export default {
  name: 'AdministrativeAuditingTable',
  ...
  methods: {
    ...
    getListData: function() {
      ...
      getAuditingListData({ ...prams, token: this.token }).then(response => {
        this.tableData = response.data.artAchiCmpAuditList
    ...
    getAuditing(id) {
      ...
      getAdministrativeDetail({ ...prams, token: this.token }).then(response => {
        this.pvData = response.data.administrativeAward
        console.log('输出PV数据')
    ...
    editAuditing: function(num) {
      ...
      auditingAdministrative({ ...prams, token: this.token }).then(response => {
        this.$message({
          message: '审核完成',
          type: 'success'
        })
