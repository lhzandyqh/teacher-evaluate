  ...
  <div class="app-container">
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="300" align="center" label="比赛名称">
        <template slot-scope="scope">
          <span>{{ scope.row.entry_name }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogPvVisible" title="审核详情">
      ...
      <el-table :data="pvData" fit highlight-current-row style="width: 100%">
        ...
        <el-table-column prop="organizers" label="主办单位"/>
...
import { getAuditingListData, getEduTeachCompetDetail, auditingEduTeachCompet } from '@/api/getAuditingData'
import imgPreview from '@/views/aTeacherGrow/auditingListTable/imgPreview'
export default {
  name: 'ApprasialAuditingTable',
  ...
  methods: {
    ...
    getListData: function() {
      ...
      getAuditingListData({ ...prams, token: this.token }).then(response => {
        this.tableData = response.data.teaEduCmpAuditList
    ...
    getAuditing(id) {
      ...
      getEduTeachCompetDetail({ ...prams, token: this.token }).then(response => {
        this.pvData = response.data.eduTeachCompet
        console.log('输出PV数据')
    ...
    editAuditing: function(num) {
      ...
      auditingEduTeachCompet({ ...prams, token: this.token }).then(response => {
        this.$message({
          message: '审核完成',
          type: 'success'
        })
